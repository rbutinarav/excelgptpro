...
```vba
' Function to retrieve setup values stored in a "Setup" sheet of the workbook
Public Function GetSetupValue(parameterName As String) As String
    ...
    ' Loop through the first column in the Setup sheet
    ' and return the value in the next column of the same row
    ' if the provided parameter name is found
    Dim cell As Range
...
Public Function OpenAI(prompt As String, Optional engine As String, Optional temperature As String, Optional max_tokens As String) As String
    ...
    ' Get API details from the Setup sheet
...
' Function to convert a range of cells in the form of a key-value pair into a JSON object
Public Function RangeToJSON(rng As Range) As String
    ...
    json = "{"
    ' Loop through the cells in the range, adding each key-value pair to the JSON object
...
' Function to convert a range of cells in the form of a table into a JSON array of objects
